...
tags: ['css', 'frontend']
draft: false
tldr: 'CSS nesting lets you write child selectors inside a parent rule so styles follow the DOM (like Sass), using the ampersand (`&`) or implicit nesting in supporting browsers. This post explains what changed now that nesting is native, when to prefer `&`, and practical patterns so you can adopt it without preprocessor-only habits.'
---
...
If you're curious about seeing CSS nesting in action, I've put together a small demo project that showcases various nesting techniques and patterns: open the [CSS nesting demo on CodePen](https://codepen.io/johanguse/pen/abRVjQZ).
